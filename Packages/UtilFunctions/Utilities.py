...
class Utilities:
    ...
    def getValuefromDict(inputDict:dict,key:str):
        ...
        if not keyValue:
            ...
            Utilities.log.error(UserException.MyError.__repr__(
        ...
        if type(attrList) is list:
            for listAttr in attrList:
                if type(listAttr) is str:
                    Utilities.getHeader(prnttag,prnttag,inputDict)
                else:
                    Utilities.parseHeader(prnttag,listAttr,inputDict)
        elif type(attrList) is collections.OrderedDict:
            for k,v in attrList.items():
                if type(v) is str:
                    Utilities.getHeader(prnttag,k,inputDict)
                else:
                    Utilities.parseHeader(k,v,inputDict)
    ...
    def parseList(prnttag: str, attrList, inputDict:dict):
        ...
        if type(attrList) is list:
            for listAttr in attrList:
                if type(listAttr) is str:
                    Utilities.getValue(prnttag,prnttag,listAttr,inputDict)
                else:
                    Utilities.parseList(prnttag,listAttr,inputDict)
        elif type(attrList) is collections.OrderedDict:
            for k,v in attrList.items():
                if type(v) is str:
                    Utilities.getValue(prnttag,k,v,inputDict)
                else:
                    Utilities.parseList(k,v,inputDict)
